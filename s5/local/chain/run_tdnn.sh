...
dir=exp/chain${nnet3_affix}/tdnn${tdnn_affix}_bi
train_data_dir=data/${train_set}_hires_comb
lores_train_data_dir=data/${train_set}_comb
train_ivector_dir=exp/nnet3${nnet3_affix}/ivectors_${train_set}_hires_comb
...
if [ $stage -le 16 ]; then
  # Build a tree using our new topology.  We know we have alignments for the data 
  # (local/nnet3/run_ivector_common.sh made them), so use those.
...
if [ $stage -le 20 ]; then
  ...
  for dset in dev-balanced test-balanced; do
      (
